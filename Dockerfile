...
RUN apt-get -y --no-install-recommends install xserver-xorg-video-qxl spice-vdagent xserver-xspice ffmpeg sudo locales at-spi2-core dialog spice-html5 websockify libxrandr-dev dirmngr libgtk2.0-0 libgtk-3-0 libsoup2.4-1 libx11-dev x11-xserver-utils exo-utils supervisor \
    ...
    && apt-get -y --no-install-recommends install pulseaudio pavucontrol git dnsutils libssl-dev libffi-dev dbus-x11 rsyslog \
